...
// ─── APEX CHATBOT ────────────────────────────────────────────────────────────
const APEX_SUGGESTIONS = [
  "What services do you offer?",
  "How much does it cost?",
  "How does AI automation work?",
  "How do I get started?"
];
const APEX_WELCOME = "Hey! I'm Apex, the AI assistant for Coded For You. I can help you learn about our AI automation services, pricing, and how we can help your business grow. What can I help you with?";

function ApexTypingDots() {
  return (
...
function ApexMessageBubble({ msg }) {
...
function Chatbot() {
  ...
  const [apexSuggestions, setApexSuggestions] = useState(true);
  const [apexUnread, setApexUnread] = useState(false);
  const [apexError, setApexError] = useState(null);
  const apexEndRef = useRef(null);
  const apexInputRef = useRef(null);
  const apexGreeted = useRef(false);

  useEffect(() => {
    apexEndRef.current?.scrollIntoView({ behavior: "smooth" });
  }, [apexMessages, apexLoading]);

  useEffect(() => {
    if (apexOpen) {
      setTimeout(() => apexInputRef.current?.focus(), 300);
      setApexUnread(false);
      if (!apexGreeted.current) {
        apexGreeted.current = true;
        setApexMessages([{ role: "assistant", content: APEX_WELCOME }]);
      }
    }
  }, [apexOpen]);

  const apexSend = useCallback(async (text) => {
    const trimmed = text.trim();
    if (!trimmed || apexLoading) return;
    setApexError(null);
    setApexSuggestions(false);
    setApexInput("");
    const userMsg = { role: "user", content: trimmed };
    const history = [...apexMessages, userMsg];
    setApexMessages(history);
    setApexLoading(true);
    try {
      ...
      setApexMessages(prev => [...prev, { role: "assistant", content: data.reply }]);
      if (!apexOpen) setApexUnread(true);
    } catch (err) {
      setApexError(err.message || "Connection error. Please try again.");
    } finally {
      setApexLoading(false);
    }
  }, [apexMessages, apexLoading, apexOpen]);

  const apexKeyDown = (e) => {
    if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); apexSend(apexInput); }
  };
  ...
  return (
    <>
      ...
      >
        <AnimatePresence mode="wait">
          {apexOpen ? (
            <motion.svg key="x" initial={{ rotate: -90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: 90, opacity: 0 }} transition={{ duration: 0.18 }} width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round">
        ...
        {apexUnread && !apexOpen && (
          <div style={{ position: "absolute", top: 0, right: 0, width: 14, height: 14, borderRadius: "50%", background: "#22c55e", border: "2px solid #050505" }} />
        )}
      </motion.button>
      ...
      <AnimatePresence>
        {apexOpen && (
          ...
          >
            {/* Header */}
            <div style={{ padding: "14px 16px", background: "rgba(0,0,0,0.4)", borderBottom: "1px solid rgba(0,180,255,0.12)", display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ width: 38, height: 38, borderRadius: "50%", background: "linear-gradient(135deg, #00B4FF, #0055CC)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#fff", fontFamily: "'Orbitron', sans-serif", position: "relative", flexShrink: 0 }}>
                A
                <div style={{ position: "absolute", bottom: 0, right: 0, width: 10, height: 10, borderRadius: "50%", background: "#22c55e", border: "2px solid #080812" }} />
              </div>
              <div style={{ flex: 1 }}>
              ...
              <button onClick={() => setApexOpen(false)} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.35)", padding: 4, borderRadius: 6, display: "flex", alignItems: "center" }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
              </button>
            </div>

            {/* Messages */}
            <div className="chat-messages" style={{ flex: 1, overflowY: "auto", padding: "16px 14px 8px" }}>
              {apexMessages.map((msg, i) => <ApexMessageBubble key={i} msg={msg} />)}
              {apexLoading && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ display: "flex", alignItems: "flex-end", gap: 8, marginBottom: 10 }}>
                  ...
                  <div style={{ padding: "10px 14px", borderRadius: "16px 16px 16px 4px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}><ApexTypingDots /></div>
                </motion.div>
              )}
              {apexError && (
                <div style={{ textAlign: "center", padding: "8px 12px", margin: "4px 0", background: "rgba(255,80,80,0.08)", border: "1px solid rgba(255,80,80,0.2)", borderRadius: 10, color: "rgba(255,120,120,0.9)", fontSize: 12, fontFamily: "'Exo 2', sans-serif" }}>{apexError}</div>
              )}
              <div ref={apexEndRef} />
            </div>
            ...
            <AnimatePresence>
              {apexSuggestions && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} style={{ padding: "0 14px 10px", display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {APEX_SUGGESTIONS.map(s => (
                    <button key={s} onClick={() => apexSend(s)}
                      style={{ background: "transparent", border: "1px solid rgba(0,180,255,0.2)", borderRadius: 20, padding: "5px 11px", color: "rgba(0,180,255,0.7)", fontSize: 11.5, cursor: "pointer", fontFamily: "'Exo 2', sans-serif", transition: "all 0.15s" }}
            ...
            {/* Input */}
            <div style={{ padding: "10px 12px", borderTop: "1px solid rgba(0,180,255,0.1)", background: "rgba(0,0,0,0.3)", display: "flex", gap: 8, alignItems: "flex-end" }}>
              <textarea
                ref={apexInputRef}
                value={apexInput}
                onChange={e => { setApexInput(e.target.value); e.target.style.height = "auto"; e.target.style.height = Math.min(e.target.scrollHeight, 96) + "px"; }}
                onKeyDown={apexKeyDown}
                placeholder="Ask about our services..."
                rows={1}
                maxLength={1000}
                style={{ flex: 1, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(0,180,255,0.15)", borderRadius: 12, padding: "9px 13px", color: "#fff", fontSize: 13.5, fontFamily: "'Exo 2', sans-serif", resize: "none", outline: "none", lineHeight: 1.45, minHeight: 40, maxHeight: 96 }}
              />
              <button
                onClick={() => apexSend(apexInput)}
                disabled={!apexInput.trim() || apexLoading}
                style={{ width: 38, height: 38, borderRadius: 10, border: "none", background: apexInput.trim() && !apexLoading ? "linear-gradient(135deg, #00B4FF, #0055CC)" : "rgba(255,255,255,0.07)", cursor: apexInput.trim() && !apexLoading ? "pointer" : "not-allowed", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all 0.2s", alignSelf: "flex-end" }}
              >
            ...
            {/* Footer */}
            <div style={{ textAlign: "center", padding: "6px 8px", borderTop: "1px solid rgba(255,255,255,0.04)", background: "rgba(0,0,0,0.2)", fontSize: 10, color: "rgba(255,255,255,0.2)", fontFamily: "'Exo 2', sans-serif" }}>
